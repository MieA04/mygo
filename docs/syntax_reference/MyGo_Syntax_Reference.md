...
关于如何构建和运行 MyGo 编译器，请参阅 [编译器指令文档](file:///d:/program/语言设计/mygo/mygo/docs/syntax_reference/CompilerInstructions-zh.md)。
...
*   **可选类型**: `T?`（在编译阶段归一化为 `Option<T>`）。例: `int?`, `Map<string,int>?`。
...
// 序列化边界特化（Phase 4）
// 当字段类型为 Option<T> 且包含 json tag 时，Go 转译类型会映射为 *T
struct Payload {
...
说明：
* 默认语义下，`T?` 统一归一化为 `Option<T>`。
* 在结构体字段同时满足“`Option<T>` + `json` Tag”时，转译层会采用 `*T` 映射以匹配 Go 的 `encoding/json` 可空字段惯例。
* 非 `json` Tag 字段保持 `Option<T>` 模型，不启用该特化。
...
宏允许在编译期操作 AST 并生成代码。宏可以通过 `target` 对象访问被注解符号的元数据。

#### 3.7.1 反射 API (Reflection API)

*   **target.name**: 符号名称 (string)。
*   **target.kind**: 符号类型 (如 "struct", "fn")。
*   **target.fields**: 结构体字段列表 (List of MetaValue)。
    *   字段属性: `f.name`, `f.type`, `f.tag`。

#### 3.7.2 辅助函数

*   **get_tag(tag_str, key)**: 解析结构体标签中的键值对。
...
### 5.3 特殊表达式
*   **Lambda**: `(x) => { return x + 1; }`
*   **Try Unwrap (`?!`)**: 对 `Result` 或 `Option` 进行传播式解包；`Result.Err` 或 `Option.None` 会触发早返回。
*   **Panic Unwrap (`?!!`)**: 对 `Result` 或 `Option` 强制解包；失败路径会触发 panic。
