...
*   **Compilation**: Functions compile to standard machine code (or Go functions in the transpiler).
*   **Calling Convention**: Arguments are typically passed via the stack or registers, depending on the backend architecture.
*   **Stack Safety**: MyGo functions operate within fixed stack frames. Infinite recursion will lead to a stack overflow panic.
...
#### Under the Hood: Variable Capture
*   **Capture by Reference**: By default, closures capture variables by reference (if they are mutable or large) or by value (if they are small and immutable).
*   **Heap Allocation**: If a captured variable escapes the function scope (e.g., the closure is returned), the compiler promotes the variable to the heap. This ensures memory safety even after the parent function returns.
*   **Implementation**: A closure is compiled into a struct containing the function pointer and a context pointer (holding captured variables).
