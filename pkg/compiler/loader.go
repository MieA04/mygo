...
func (l *PackageLoader) LoadPackage(importPath string) (*Package, error) {
	...
	// Try loading as MyGo package (Single File)
	if err == nil && !info.IsDir() && strings.HasSuffix(info.Name(), ".mygo") {
		pkg := &Package{
			Name:       "main", // Default, will be updated by parse
		...
		file, pkgName, err := l.parseFile(dirPath)
		if err != nil {
			return nil, err
		}
		if pkgName != "" {
		...
		if err := l.processPackage(pkg); err != nil {
			return nil, err
		}
		return pkg, nil
	}

	// Try loading as MyGo package (Directory)
	if err == nil && info.IsDir() {
		...
		for _, entry := range entries {
			if !entry.IsDir() && strings.HasSuffix(entry.Name(), ".mygo") {
				...
				file, pkgName, err := l.parseFile(path)
				if err != nil {
					return nil, err
				}
				if pkgName != "" {
...
func (l *PackageLoader) parseFile(path string) (*SourceFile, string, error) {
...
func (l *PackageLoader) processPackage(pkg *Package) error {
	...
	for _, file := range pkg.Files {
		for _, spec := range file.Imports {
			...
			if err != nil {
				return fmt.Errorf("failed to load import '%s' in %s: %w", imp, file.Path, err)
			}
