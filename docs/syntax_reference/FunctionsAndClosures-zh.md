...
#### 底层原理：栈帧 (Under the Hood: Stack Frames)
*   **编译**：函数会被编译为标准机器码（在转译模式下则为 Go 函数）。
*   **调用约定**：参数通常通过栈或寄存器传递，具体取决于后端架构。
*   **栈安全**：MyGo 函数在固定大小的栈帧中运行。无限递归会导致栈溢出 Panic。
...
#### 底层原理：变量捕获 (Under the Hood: Variable Capture)
*   **引用捕获**：默认情况下，闭包通过引用捕获可变变量，或通过值捕获小型不可变变量。
*   **堆分配**：如果捕获的变量逃逸出函数作用域（例如闭包被返回），编译器会将该变量提升到堆上。这确保了父函数返回后内存依然安全。
*   **实现**：闭包在底层被编译为一个包含函数指针和上下文指针（持有捕获变量）的结构体。
